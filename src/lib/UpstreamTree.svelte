<script lang="ts">
    ...
    $: fun_children = tree_type == 'upstream' ? (x: WfTree) => x.data.input_refs : (x: WfTree) => x.data.output_refs;
	...
	{#if Math.min(fun_children(wf_node).length, max_level) > 0}
		<ul class="list-none pl-4">
			{#each fun_children(wf_node) as child, idx}
				<li><svelte:self wf_node={child} tree_type={tree_type} max_level={max_level-1} /></li>
			{/each}
		</ul>
	{/if}
</div>
